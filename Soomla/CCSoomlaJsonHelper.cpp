...
Ref *CCSoomlaJsonHelper::getRefFromJson(json_t *obj) {
    if (obj == NULL) {
        return NULL;
    }
    ...
    if (json_is_object(obj)) {
        ...
        while (iter) {
            ...
            dictionary->setObject(CCSoomlaJsonHelper::getRefFromJson(value), string(key));
        ...
        for (unsigned int i = 0; i < sizeArray; i++) {
            array->addObject(CCSoomlaJsonHelper::getRefFromJson(json_array_get(obj, i)));
        }

        return array;
    }
    else if (json_is_boolean(obj)) {
        __Bool *__Bool = __Bool::create(json_is_true(obj));
        return __Bool;
    }
        ...
        __Double *__Double = __Double::create(realVal);
        return __Double;
    }
...
json_t*CCSoomlaJsonHelper::getJsonFromRef(Ref* obj) {
    if (dynamic_cast<__Dictionary *>(obj)) {
        ...
        for (unsigned int i = 0; i < allKeys->count(); i++) {
            ...
            json_object_set_new(jsonDict,
                    key,
                    CCSoomlaJsonHelper::getJsonFromRef(mainDict->objectForKey(key)));
        }
        ...
        for (unsigned int i = 0; i < mainArray->count(); i++) {
            json_array_append_new(jsonArray,
                    CCSoomlaJsonHelper::getJsonFromRef(mainArray->getObjectAtIndex(i)));
        }
